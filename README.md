...
The REST API documentation can be found on [github.com](https://github.com/dimus). The full API of this library can be found in [api.md](api.md).
...
```sh
npm install git+ssh://git@github.com:stainless-sdks/GNverifier-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install GNverifier`
...
1. Via the `G_NVERIFIER_LOG` environment variable
...
const client = new GNverifier({
  logger: logger.child({ name: 'GNverifier' }),
...
const client = new GNverifier({ fetch });
```
...
```ts
import GNverifier from 'npm:GNverifier';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/GNverifier-typescript/issues) with questions, bugs, or suggestions.
