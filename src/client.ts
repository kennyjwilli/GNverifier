...
export interface ClientOptions {
  /**
   * Defaults to process.env['G_NVERIFIER_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['G_NVERIFIER_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['G_NVERIFIER_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the G Nverifier API.
 */
export class GNverifier {
  apiKey: string | null;
   ...
   * API Client for interfacing with the G Nverifier API.
   *
   * @param {string | null | undefined} [opts.apiKey=process.env['G_NVERIFIER_API_KEY'] ?? null]
   * @param {string} [opts.baseURL=process.env['G_NVERIFIER_BASE_URL'] ?? https://verifier.globalnames.org/api/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('G_NVERIFIER_BASE_URL'),
    apiKey = readEnv('G_NVERIFIER_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://verifier.globalnames.org/api/v1`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? GNverifier.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('G_NVERIFIER_LOG'), "process.env['G_NVERIFIER_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://verifier.globalnames.org/api/v1';
  }
  ...
  protected stringifyQuery(query: Record<string, unknown>): string {
    return Object.entries(query)
      ...
      .map(([key, value]) => {
        ...
        throw new Errors.GNverifierError(
  ...
  static GNverifier = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static GNverifierError = Errors.GNverifierError;
